/*
While Rust chooses how to capture variables on the fly mostly
without type annotation, this ambiguity is not allowed when writing
functions. When taking a closure as an input parameter, the closure's
complete type must be annotated using one of a few traits. In order of
...
that the closure may capture by &T, &mut T, or T, but the compiler will
...
annotated as Fn, then capturing variables by &mut T or T are not allowed.
